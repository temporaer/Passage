...
model = RNN(layers=layers, updater=updater, iterator='linear', cost='csce', Y=Y_)
...
tr_acc = np.mean(trY[:len(teY)] == np.argmax(tr_preds, axis=2).T)
te_acc = np.mean(teY == np.argmax(te_preds, axis=2).T)
